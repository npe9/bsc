...
findBluespecDir = do
    -- First check if BLUESPECDIR is set in environment
    maybeBluespecDir <- System.Environment.lookupEnv "BLUESPECDIR"
    case maybeBluespecDir of
        Just dir -> return dir
        Nothing -> do
            -- Check if we're in a Homebrew environment
            maybeHomebrewPrefix <- System.Environment.lookupEnv "HOMEBREW_PREFIX"
            case maybeHomebrewPrefix of
                Just prefix -> do
                    -- Check if the Bluespec directory exists in Homebrew prefix
                    let homebrewPath = prefix ++ "/lib/" ++ bluespec
                    exists <- doesDirExist homebrewPath
                    if exists
                        then return homebrewPath
                        else checkPathsInOrder [homebrewPath]
                Nothing -> checkPathsInOrder []
    where
        checkPathsInOrder [] = do
            -- If no path exists, use cabal as last resort
            home <- System.Environment.getEnv "HOME"
            return $ home ++ "/.cabal/lib/" ++ bluespec
        checkPathsInOrder (path:rest) = do
            exists <- doesDirExist path
            if exists
                then return path
                else checkPathsInOrder rest
